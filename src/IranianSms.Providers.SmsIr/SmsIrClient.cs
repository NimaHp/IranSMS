    ...
    public sealed class SmsIrClient : ISmsClient, ISmsBulkSender, ISmsOtpSender, ISmsDeliveryReporter
    {
            ...
            where TData : class
        {
            ...
            if (envelope is null || envelope.Status != 1)
            {
                if (envelope is null)
                {
                    throw new IranianSmsException("SMS.ir returned an unparseable envelope.")
                    {
                        ProviderName = "SmsIr",
            ...
            return envelope.Data ?? throw new IranianSmsException("SMS.ir API error: empty data payload.")
            {
                ProviderName = "SmsIr",
                RawResponseBody = body,
            };
